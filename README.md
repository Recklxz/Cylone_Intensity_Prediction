...
1. Go to the app directory:
   - `cd Cyclone_Intensity_Prediction`
2. Create and activate a virtual environment:
   - Windows PowerShell: `python -m venv .venv` then `.venv\Scripts\Activate.ps1`
3. Install dependencies:
   - `pip install -r requirements.txt`
4. Optional environment variables (for database connectivity):
   - `DB_HOST` (default: `localhost`)
   - `DB_NAME` (default: `cyclone_intensity`)
   - `DB_USER` (default: `postgres`)
   - `DB_PASSWORD` (default: empty string)
   - `FLASK_DEBUG` (`true` or `false`, default `false`)
5. Ensure the model weights file exists:
   - `Cyclone_Intensity_Prediction/saved_modelcyclone`
6. Run the app:
   - `python main.py`

The app still performs intensity prediction without a database connection, but archive features require PostgreSQL.
