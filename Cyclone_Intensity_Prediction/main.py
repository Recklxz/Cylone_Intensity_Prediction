...
model = Model()
model_path = os.path.join(BASE_DIR, "saved_modelcyclone")
if not os.path.exists(model_path):
    raise FileNotFoundError(
        "Missing model weights file 'saved_modelcyclone'. Place it in the app directory before running."
    )
...
DB_NAME = os.getenv("DB_NAME", "cyclone_intensity")
...
def predict():
    ...
    if imagefile and allowed_img(imagefile.filename):
        ...
        if cursor is not None:
            ...
            flash('Database not configured. Prediction works, but archive entry was not saved.')
